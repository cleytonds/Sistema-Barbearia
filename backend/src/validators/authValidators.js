...
const passwordRules = (field) =>
  body(field)
    ...
    .withMessage('deve ter entre 8 e 72 caracteres')
    .matches(/[A-Za-zÀ-ÿ]/)
    .withMessage('deve conter uma letra')
    .matches(/\d/)
...
export const registerValidator = [
  body().custom((value) => {
    ...
    if (Object.keys(value).some((key) => !allowed.includes(key)))
      throw new Error('contém campos não permitidos');
    return true;
  }),
  body('nome')
    .isString()
    ...
    .withMessage('deve ter entre 3 e 150 caracteres')
    .matches(/^[\p{L}\p{M}' -]+$/u)
    .withMessage('contém caracteres inválidos'),
  body('email').isString().trim().isLength({ max: 254 }).isEmail().withMessage('inválido'),
  body('telefone')
    .isString()
    .customSanitizer((value) => value.replace(/\D/g, ''))
    .isLength({ min: 10, max: 11 })
    .withMessage('deve conter DDD e 10 ou 11 dígitos'),
  passwordRules('senha'),
  body('confirmacaoSenha')
    .custom((value, { req }) => value === req.body.senha)
    .withMessage('não confere com a senha'),
];
...
export const resetPasswordValidator = [
  ...
  body('confirmacaoNovaSenha')
    ...
    .withMessage('não confere com a nova senha'),
];
